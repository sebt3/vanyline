...
| `.claude/memory/vscode-ext-sequence.md` | **F1+F2 closes, F3 à démarrer.** Extension VS Code `vanyline` : composant utilisable (chat + config + sandboxes) réutilisant le frontend via `packages/protocol` + `packages/ui`. **5 features**, ordre F1→F2→F3→(F4‖F5). Ce fichier porte les décisions structurantes, l'état d'avancement et la procédure de reprise. Remplace `ws06-vscode-ext-bootstrap`. |
...
- **Extension VS Code `vanyline`** — **F1 close (2026-08-31) et F2 close (2026-09-02),
  toutes deux mergées dans `main` et poussées. F3 à démarrer.**
  ...
  delete}` + actions test/localTools. Review Phase 3 : 3 bugs corrigés (cf.
  `.claude/memory/F2-vscode-ext-cli-rpc.md`).
  Séquence F3→F5 et procédure de reprise : `.claude/memory/vscode-ext-sequence.md`.
