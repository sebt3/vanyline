...
| `.claude/memory/vscode-ext-sequence.md` | **En cours — Phase 1 seule.** Extension VS Code `vanyline` redéfinie le 2026-08-29 : composant utilisable (chat + config + sandboxes) réutilisant le frontend web via `packages/protocol` + `packages/ui`. Découpée en **5 features** (`docs/features/F{1..5}-vscode-ext-*.md`), ordre F1→F2→F3→(F4‖F5). Les 5 design docs écrits, aucune implémentation démarrée. **Ce fichier porte les décisions structurantes, l'état d'avancement et la procédure pour reprendre Phase 3 / feature suivante en session neuve.** Remplace `ws06-vscode-ext-bootstrap` (supprimé) |
...
- **Extension VS Code `vanyline`** — Phase 1 des 5 features écrite le 2026-08-29
  (`docs/features/vscode-ext-*.md`), implémentation pas démarrée. Redéfinition complète
  de l'ancien `vscode-ext-bootstrap` : composant utilisable (chat + config + sandboxes)
  réutilisant le frontend via `packages/protocol` + `packages/ui`. Décisions, séquence
  F1→F5, procédure de reprise : `.claude/memory/vscode-ext-sequence.md`.
