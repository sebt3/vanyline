...
| `.claude/memory/vscode-ext-sequence.md` | **F1 close, F2 à démarrer.** Extension VS Code `vanyline` : composant utilisable (chat + config + sandboxes) réutilisant le frontend via `packages/protocol` + `packages/ui`. **5 features** (`docs/features/F{2..5}-vscode-ext-*.md`), ordre F1→F2→F3→(F4‖F5). Ce fichier porte les décisions structurantes, l'état d'avancement et la procédure de reprise. Remplace `ws06-vscode-ext-bootstrap`. |
| `.claude/memory/F1-vscode-ext-foundations.md` | **F1 (close 2026-08-31).** `@vanyline/protocol` (ts-rs `ChatEvent` + enveloppes RPC + `RpcConnection` + `config-domain.ts` miroir manuel de `domain.rs`) et `@vanyline/ui` (chat + 6 écrans config + `ConfigShell`, agnostiques du backend via 3 ports injectés `ChatTransport`/`ChatBackend`/`ConfigRepo`) extraits du frontend. `frontend/` implémente les ports (`httpConfigRepo` traduit REST↔canonique : `type`↔`provider_type`, FK id↔nom). 2 blocages Phase 2 tranchés en session (extraction chat → port `ChatBackend` ; refs inter-domaines → noms partout). Délégation Qwen sur task 06 : timeout à ~70 %, fini par Claude. Le reste fait directement par Claude (choix du développeur après le raté Qwen). Review Phase 3 : chat extraction (Cadence) + config (Claude) propres, pas de bug bloquant. `domain.rs::McpTransport` doit gagner `Sse` en F2. |
...
- **Extension VS Code `vanyline`** — **F1 close (2026-08-31)**, F2 à démarrer.
  `@vanyline/protocol` + `@vanyline/ui` extraits du frontend (chat + 6 écrans config +
  `ConfigShell`, agnostiques du backend via ports injectés). Branche
  `feat/F1-vscode-ext-foundations` **pas encore poussée/mergée**. Bilan :
  `.claude/memory/F1-vscode-ext-foundations.md` ; séquence F2→F5 et procédure de
  reprise : `.claude/memory/vscode-ext-sequence.md`. Comme le reste de la famille : pas
  testé sur cluster réel (pas de backend dans l'env de dev).
