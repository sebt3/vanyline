...
mod tests {
    ...
    fn export_chat_event_ts() {
        ...
        let chat = <ChatEvent as TS>::export_to_string(&cfg).unwrap();
        let record = <ToolCallRecord as TS>::export_to_string(&cfg).unwrap();
        let out = format!(
        ...
        let target = concat!(
            ...
            "/../packages/protocol/src/generated/chat-event.ts"
        );
